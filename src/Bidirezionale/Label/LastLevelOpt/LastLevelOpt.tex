...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \STATE $sourceRepaired \leftarrow$ false
        \STATE $sinkRepaired \leftarrow$ false
        \IF{$noCapsSource.isEmpty$}
...
\begin{algorithm}
    \begin{algorithmic}
        ...
        \ENDFOR
        \ENDIF
        \STATE $sourceRepaired \leftarrow repaired$
        \IF{$\neg repaired$}
        \IF{$noCapSource = s$}
...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \IF{$n.flussoPassante \ne 0$} \COMMENT{il nodo è già stato visitato}
        \IF{$n.sourceside$}
        \STATE \textbf{continue}
        \ELSE
        ...
        \STATE addLast$(n)$ \COMMENT{aggiunge n a LastSinkNodes e i nodi a lui collegati sourceSide li inserisco in LastSourceNodes}
