...
using LLOGraph = Bidirezionale.NodeCount.LastLevelOptEdgeFlow.Graph;
using LLONode = Bidirezionale.NodeCount.LastLevelOptEdgeFlow.Node;
...
namespace MaxFlow
{
    public class RandomProgram
    {
        public static (int, NOGraph, LLOGraph, SPGraph) CreateGraph(int cardNodes, int seed)
        {
            ...
            NONode sNO = new Bidirezionale.NodeCount.NoOpt.SourceNode("0");
            LLONode sLLO = new Bidirezionale.NodeCount.LastLevelOptEdgeFlow.SourceNode("0");
            SPNode sSP = new Bidirezionale.NodeCount.SickPropagation.SourceNode("0");
            grafoNO.AddNode(sNO);
            nodesNO.Add(sNO);
            ...
            var tNO = new Bidirezionale.NodeCount.NoOpt.SinkNode(cardNodes.ToString());
            var tLLO = new Bidirezionale.NodeCount.LastLevelOptEdgeFlow.SinkNode(cardNodes.ToString());
            var tSP = new Bidirezionale.NodeCount.SickPropagation.SinkNode(cardNodes.ToString());
            grafoNO.AddNode(tNO);
            nodesNO.Add(tNO);
        ...
        static void Main(string[] args)
        {
            ...
            watch.Restart();
            var res2 = BiNodeCountLastLevelOpt.FlowFordFulkerson(graphLLO);
            watch.Stop();
