...
using LLOGraph = Bidirezionale.NodeCount.LastLevelOpt.Graph;
using LLONode = Bidirezionale.NodeCount.LastLevelOpt.Node;
using NoOptGraph = Bidirezionale.NodeCount.NoOpt.Graph;
using NoOptNode = Bidirezionale.NodeCount.NoOpt.Node;
using SPGraph = Bidirezionale.NodeCount.SickPropagation.Graph;
using SPNode = Bidirezionale.NodeCount.SickPropagation.Node;



namespace Bidirezionale.NodeCount
{
    public class Program
    {
        private static (NoOptGraph, LLOGraph, EFGraph, SPGraph) Read(string[] stringhe)
        {
            ...
            if (string.Equals(s[0], "n"))
                ...
                {
                    grafoNoOpt.AddNode(new Bidirezionale.NodeCount.NoOpt.SourceNode(s[1]), false);
                    grafoLLO.AddNode(new Bidirezionale.NodeCount.LastLevelOpt.SourceNode(s[1]), false);
                    grafoEF.AddNode(new Bidirezionale.NodeCount.LastLevelOptEdgeFlow.SourceNode(s[1]), false);
                    grafoSP.AddNode(new Bidirezionale.NodeCount.SickPropagation.SourceNode(s[1]), false);

                }
                else if (String.Equals(s[2], "t"))
                {
                    grafoNoOpt.AddNode(new Bidirezionale.NodeCount.NoOpt.SinkNode(s[1]), true);
                    grafoLLO.AddNode(new Bidirezionale.NodeCount.LastLevelOpt.SinkNode(s[1]), true);
                    grafoEF.AddNode(new Bidirezionale.NodeCount.LastLevelOptEdgeFlow.SinkNode(s[1]), true);
                    grafoSP.AddNode(new Bidirezionale.NodeCount.SickPropagation.SinkNode(s[1]), true);
                }
            s = stringhe[3].Split(" ");
            if (String.Equals(s[0], "n"))
                if (String.Equals(s[2], "s"))
        ...
        public static void Performance(NoOptGraph graphNN, LLOGraph graphLLO, EFGraph graphEF, SPGraph graphSP)
        {
            ...
            watch.Start();
            var res1 = BiNodeCountNoOpt.FlowFordFulkerson(graphNN);
            watch.Stop();
            ...
            watch.Restart();
            var res4 = LastLevelOptEdgeFlow.BiNodeCountLastLevelOpt.FlowFordFulkerson(graphEF);
            watch.Stop();
